...
| Timeout / Delay      | None                                                                                                                                                                                                                                                      | ```delay(obj : { duration: number }): Future<T>``` <br>  ``` completeBefore(obj: { timeOut: number,  orElse?: () => Future<T> }): Future<T>``` |
| Callback             | None (but can be simulated with then/catch/finally methods)                                                                                                                                                                                               | ```onComplete(fn : () => void): void ``` <br>  ``` onComplete(fn : { ifSuccess: (result: T) => void,  ifFailure: (exception: Exception) => void}): void ``` <br>  ```onSuccess(fn : T => void) : void``` <br>  ```onFailure(fn: Exception => void): void```|
