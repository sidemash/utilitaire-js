...
| Static Method Creation | ```Promise.resolve<T>(value:T) : Promise<T>```<br> ```Promise.reject<T>(err:any):Promise<T>``` <br><br><br><br>  | ```Future.successful<T>(value:T): Future<T>```<br> ```Future.failed<T>(exception:Exception): Future<T>```<br> ```Future.foreverPending<T>() : Future<T>```<br> ```Future.notYetStarted<T>(): Future<T>```                                                                                                                                                                  |
